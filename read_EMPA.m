function EMPA = read_EMPA(EMPA_File,INCAS)
...
for i=1:size(EMPA,1)
    [~,INCAS_Index] = min(abs(datenum(INCAS.Date)-datenum(EMPA.Date(i))));
    EMPA.Dis2(i) = INCAS.Dis2(INCAS_Index);
end

for i = 1:size(EMPA,1)-1
    EMPA.Dis (i) = EMPA.Dis2(i+1) - EMPA.Dis2(i);
end
EMPA.Dis(i+1) = EMPA.Dis(i);
